const urlParams = new URLSearchParams(window.location.search);
	...
	function displayIPData(data) {
		...
		document.getElementById('locationLine1').innerHTML = `Lat: ${data.loc.split(',')[0]}<br>City: ${data.city}<br>Organization: ${data.org}`;

		document.getElementById('locationLine2').innerHTML = `Long: ${data.loc.split(',')[1]}<br>Region: ${data.region}<br>Hostname: ${data.hostname || 'N/A'}`;
		...
		document.getElementById('datetime').textContent = `Data And Time: ${currentTime}`;
		
		document.getElementById('pincode').textContent = `Pincode: ${data.postal}`;
		document.getElementById('message').textContent = `Message: ${data.postal.length}`
		;
